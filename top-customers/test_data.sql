create table customers (customer_id integer primary key, name varchar(255) not null) ;

create table orders (order_id integer primary key, customer_id integer not null references customers(customer_id), ordered_at timestamp not null) ;

create table products (product_id integer primary key, name varchar(255) not null) ;
...
SELECT YEAR(orders.ordered_at) as year, 
 MONTH(orders.ordered_at) as month,
 orders.customer_id ,orders.order_id,
 MAX(order_line_items.unit_price *  order_line_items.quantity)  as total_monthly_order_value  FROM order_line_items 
